...
from trying_experiments import ColoringYOLO
...
checkpoint_example_weights = 'checkpoints/50.weights'
...
parser.add_argument("--weights_path", type=str, default=kitti_weights, help="path to weights file/ can use darknet53.conv.74")
#parser.add_argument("--weights_path", type=str, default=checkpoint_example_weights, help="path to weights file/ can use darknet53.conv.74")
...
# Initiate model
darknet_model = Darknet(opt.model_config_path)
#model.apply(weights_init_normal)
darknet_model.load_weights(opt.weights_path)
#model.apply(weights_init_normal)


model = ColoringYOLO(darknet_model)
...
def save_color_layer(layer, path='weights/color_layer.weights'):
...
def do_training():
    ...
    test_dataloader = torch.utils.data.DataLoader(test_dataset, batch_size=1, shuffle=False, num_workers=opt.n_cpu) 

    if freeze_except_first:
        color_layer = model.conv_color
        yolo_block = model.darknet
        for i, (name, p) in enumerate(color_layer.named_parameters()):
            p.requires_grad = True
        for i, (name, p) in enumerate(yolo_block.named_parameters()):
            p.requires_grad = False

    for epoch in range(opt.epochs):
        start = time.time()
        losses_x = losses_y = losses_w = losses_h = losses_conf = losses_cls = losses_recall = losses_precision = batch_loss= 0.0
        if freeze_except_first: # Do nothing
            pass
        else: # Set trainable parameters as usual according to other freezing options
        ...
        if epoch % opt.checkpoint_interval == 0:
            ...
            save_color_layer(model.conv_color, f"weights/color_layer{epoch}.weights")
        ...
        if(mAP > best_mAP):
            best_mAP = mAP
            save_color_layer(model.conv_color, "weights/best_color_1c.weights")
